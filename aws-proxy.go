...
// Code taken from https://github.com/pottava/aws-s3-proxy
// Needs to be reworked into gin framework
// Auth needs to be replaced by TasadarAuth
// Edit ProxyAuth to depend on auth for this
...
func awsProxy(router *gin.Engine) {
	...
	log.Printf("[AWS-Proxy] listening on %s", addr)
	if (len(c.sslCert) > 0) && (len(c.sslKey) > 0) {
		go log.Fatal(http.ListenAndServeTLS(addr, c.sslCert, c.sslKey, nil))
	} else {
...
func wrapper(f func(w http.ResponseWriter, r *http.Request)) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if (len(c.basicAuthUser) > 0) && (len(c.basicAuthPass) > 0) && !proxyauth(r) && !isHealthCheckPath(r) {
			w.Header().Set("WWW-Authenticate", `Basic realm="Tasadar"`)
